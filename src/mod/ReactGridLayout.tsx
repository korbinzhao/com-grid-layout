...
export default class ReactGridLayout extends React.Component<Props, State> {
   ...
   */
  onDragStart(i: string, x: number, y: number, { e, node }) {
   ...
   * @param {Element} node The current dragging DOM element
   */
  onDrag(i: string, x: number, y: number, { e, node }) {
   ...
   * @param {Element} node The current dragging DOM element
   */
  onDragStop(i: string, x: number, y: number, { e, node }) {
  ...
  onResizeStart(i: string, w: number, h: number, { e, node }) {
    const { layout } = this.state;
    var l = getLayoutItem(layout, i);
    if (!l) return;
  ...
  onResize(i: string, w: number, h: number, { e, node }) {
  ...
  onResizeStop(i: string, w: number, h: number, { e, node }) {
    const { layout, oldResizeItem } = this.state;
    const { cols } = this.props;
    var l = getLayoutItem(layout, i);
