 ...
 */
export function bottom(layout): number {
  let max = 0,
    bottomY;
 ...
 */
export function childrenEqual(a, b): boolean {
  return isEqual(
 ...
 */
import _fastRGLPropsEqual from "./fastRGLPropsEqual"
export const fastRGLPropsEqual = _fastRGLPropsEqual ;
 ...
 */
export function collides(l1, l2): boolean {
 ...
 */
export function getLayoutItem(layout, id: string) {
  for (let i = 0, len = layout.length; i < len; i++) {
...
export function compactType(
  props: { verticalCompact: boolean, compactType }
) {
